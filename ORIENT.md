...
- 🛑 **`fatal: mmap failed: Operation canceled` is iCLOUD, not file size.** This was blamed on the 985 KB `index.html` for a long time; it is not. A clone under `~/Desktop` (or `~/Documents`) is inside **iCloud Drive's Desktop & Documents sync**, so git writes a pack into `.git/objects/pack/` and then `mmap`s it to index it — and iCloud hands back a placeholder instead of bytes, which surfaces as `ECANCELED`. Confirmed 2026-09-08: fetching into `/Users/brianhoward/Desktop/labor-dashboard` failed every time, including with `core.packedGitWindowSize`/`packedGitLimit`/`pack.threads` turned right down, while a plain `git clone` into `~/dev/labor-dashboard` resolved 3910 objects and 2552 deltas without complaint. **Keep the working clone outside `~/Desktop` and `~/Documents`.** The big files only make it hit sooner; they are not the cause, and tuning git's memory does nothing.
- **`git worktree` full checkout** hits the same wall from the same place. Outside iCloud it is fine; if you are stuck inside it, `git worktree add --no-checkout … && git -C <wt> checkout HEAD -- index.html tailwind.css` still works. That leaves other files as phantom "deletions" in the worktree — commit with `git commit --only index.html` so you don't commit those deletions.
...
- **To put a page on the phone, put it in the sidebar.** The bar's last tab opens the Menu page (`#page-menu`), which is generated from the sidebar on every visit — there is no second list to edit. A top-level sidebar item needs `data-menu-section`; `npm test` fails without it. The catch: a sidebar item a role can see but `navigateToPage` refuses is a dead row on the phone, so gate the sidebar exactly as the router does, and run `scripts/browser-mobile-menu.mjs`, which opens every row for every role.
